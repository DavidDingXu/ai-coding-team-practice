...
| 文件或目录 | Owner | 生命周期 | 规则 |
|---|---|---|---|
| `.current-task` | 控制面 | active task 期间 | 只保存当前任务 ID，归档后清理 |
| `task.json` | 控制面 | active + archive | 只保存流程状态和恢复必要事实 |
| `prd.md` / `design.md` / `plan.md` / `summary.md` | 对应阶段 Agent 或负责人 | 稳定产物 | 人可以审查，不能被 builder 随意覆盖 |
| `generated/context/*` | context builder | 可重建 | 缺失或过期时重建，不作为真相源 |
| `evidence/*` | worker / verifier | 追加型证据 | 不覆盖历史记录，失败也要记录 |
| `spec/**` | 团队或应用 owner | 长期资产 | 需要来源、适用范围和维护入口 |
| `archive/index.jsonl` | 归档流程 | 长期索引 | 用于后续弱召回，不替代当前需求 |
...
| L2 状态驱动 | 有 `task.json`、当前任务指针、阶段状态 | 多人接力 |
